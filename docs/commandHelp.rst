...
This command requires sassy <https://github.com/RagnarGrootKoerkamp/sassy>. :code:`sassy` should be either on the user's PATH or the path to the sassy executable should be set as a configuration variable
...
:code:`dataCol [-datacol i ] [-transf identity] [-aggfun mean] [-v] [track_regex = .*]...`

Set data column for bedgraph tracks 

Set which column to plot, what transformation to apply, and how to aggregate values when multiple values fall in the same screen column. Non-numeric data, including any generated NaN, are ignored.

:code:`-datacol` Column index (1-based) of data to plot.

:code:`-transf` Apply this transformation to the data. This happens before aggregation. Options: [IDENTITY, LOG10, MINUS_LOG10]

:code:`-aggfun` Aggregate function to summarise data falling in the same screen column. Options: [MEAN, MAX, MIN]
...
Parameters and current settings::
    ...
    sassy                              null # Path of sassy executable                                                                          
