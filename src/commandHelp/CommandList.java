...
public class CommandList {
	...
	public final static List<CommandHelp> commandHelpList() throws InvalidCommandLineException, InvalidColourException{
		...
		cmd.setAdditionalDescription("awk offers finer control then :code:`grep` to filter records in tabular format.\n"
				...
				+ "*ADDITIONAL FEATURES*\n"
				+ "\n"
				+ "Function :code:`get(...)` can indistinctly be applied to get tags from SAM records, and from INFO "
				+ "and FORMAT fields in VCF files. Double quoting around <tag> is optional."
				+ "\n"
				+ "* :code:`get(tag)` When applied to SAM files, return the value of the given sam tag."
				+ "A record is filtered out if the tag is not found.\n"
				+ "\n"
				+ "* :code:`get(tag, [value_index])` When applied to VCF files, return the value of the given "
				...
				+ "contain this tag.\n"
				+ "\n"
				+ "* :code:`get(tag, [sample_idx], [value_idx])` When applied to VCF files, "
				+ "return the value of the FORMAT tag for sample index *sample_idx* (default to 1, first sample). "
				+ "if the tag contains multiple values, optionally return the value at index *value_idx*. "
				+ "If necessary, prepend 'FMT/' to tag to disambiguate it from INFO tags or if the header does not "
				+ "contain this tag.\n"
				+ "\n"
				...
				+ "    awk 'get(NM) > 0' .bam\n"
				+ "\n"
				+ "* Filter vcf records for value of DP tag in second sample, first item::\n"
				+ "\n"
				+ "    awk 'get(DP, 2, 1) > 0' .vcf.gz\n"
				+ "\n"
