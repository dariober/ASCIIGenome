...
public class SystemCommandTest {
  ...
  public void canExecSystemCommand() throws IOException, InterruptedException {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughSystemCommand(Stream.of(input), null, "awk -v FOO=5 '$3 > FOO' | grep bar | cat")) {
    ...
    boolean pass = false;
    try (Stream<String> s = sysCmd.streamLinesThroughSystemCommand(Stream.of(input), null, "foobar")) {
  ...
  public void canStreamLinesThroughSystemCommand() throws IOException, InterruptedException {
    ...
    try (Stream<String> stream = sysCmd.streamLinesThroughSystemCommand(Stream.of(input),
    ...
    try (Stream<String> stream = sysCmd.streamLinesThroughSystemCommand(Stream.of(new String[]{}),
        null, "")) {
      List<String> res = stream.toList();
      assertTrue(res.isEmpty());
    }
    ...
    boolean pass = false;
    try (Stream<String> ignored = sysCmd.streamLinesThroughSystemCommand(Stream.of(input),
  ...
  public void canFilterUsingAwk() throws IOException {
    ...
    SystemCommand sysCmd = new SystemCommand();
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(in3), "{VAR=5} $2 > VAR && $1")) {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(in4), "{VAR=5} $2 > VAR && $1")) {
      List<String> results = s.toList();
    ...
    String[] in2 = {};
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(in2), "{VAR=5} $2 > VAR && $1")) {
      List<String> results = s.toList();
      assertEquals(0, results.size());
    }
  ...
  public void canFilterGtfAwkFunc() throws IOException {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(gtf), "getGtfTag(\"gene_id\") == 100")) {
      List<String> results = s.toList();
      assertEquals(gtf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(gtf), "getGtfTag(\"trax_id\") == \"ACTB\"")) {
      List<String> results = s.toList();
      assertEquals(gtf[0], results.get(0));
    }
    // Empty string if key not found
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(gtf), "getGtfTag(\"SPAM\") == \"\"")) {
      List<String> results = s.toList();
      assertEquals(gtf[0], results.get(0));
    }
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(gtf), "getGtfTag(\"gene_id\") == \"\"")) {
      List<String> results = s.toList();
      assertEquals(gtf[0], results.get(0));
    }
  ...
  public void canFilterGffAwkFunc() throws IOException {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Tag\") == 100")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"ID\") == \"foo : bar\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Alias\") == \"spam,bar\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Alias\") == \"spam,bar\"")) {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Alias\", 1) == \"spam\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Alias\", 2) == \"bar\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Alias\", 99) == \"\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"SPAM\") == \"\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }

    // NB: Missing tag i.e., empty string, evaluates to 0!!
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"SPAM\") == 0")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }

    x = new String[] {".|.|.|.|.|.|.|.|.".replaceAll("\\|", "\t")};
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Alias\") == \"\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(x), "getGffTag(\"Tag\") == \"\\\"X\\\"\"")) {
      List<String> results = s.toList();
      assertEquals(x[0], results.get(0));
    }
  }
  ...
  public void canFilterInfoVcfWithAwkFunc() throws IOException {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"IMPRECISE\") == 1")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"ABSENT_TAG\") == 0")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"FOLD_CHANGE\") <= 0.723657")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"FOLD_CHANGE\") > 0.723657")) {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"DP\") == \"20,30\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"DP\", 1) == 20")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"DP\", 2) == 30")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }

    // Out of range
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"DP\", 3) == \"\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getInfoTag(\"FOO\") == 0")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
  ...
  public void canFilterFormatVcfWithAwkFunc() throws IOException {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GT\") == 11")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0)); // Default sample_idx= 1
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GT\", 1) == 11")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GT\", 2) == 22")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GT\", 2) < 22")) {
    ...
    // Get value from list
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GQ\", 2) == \"99,100\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GQ\", 2, 1) == \"99\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GQ\", 2, 2) == \"100\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    // Out range
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GQ\", 2, 3) == \"\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }

    // Tag not found
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"ABSENT\", 1) == \"\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }

    // Invalid indexes
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GT\", 99) == \"\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(vcf), "getFmtTag(\"GT\", \"foobar\") == \"\"")) {
      List<String> results = s.toList();
      assertEquals(vcf[0], results.get(0));
    }
  }
  ...
  public void canFilterSamTagWithAwk() throws IOException {
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(rec), "getSamTag(\"NH\") > 10")) {
      List<String> results = s.toList();
      assertEquals(0, results.size());
    }
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(rec), "{getSamTag(\"ZZ\"); print $0}")) {
      List<String> results = s.toList();
      assertEquals(rec[0], results.get(0));
    }
    ...
    try (Stream<String> s = sysCmd.streamLinesThroughAwk(Stream.of(rec), "getSamTag(\"NM\") == \"\"")) {
      List<String> results = s.toList();
      assertEquals(rec[0], results.get(0));
    }
