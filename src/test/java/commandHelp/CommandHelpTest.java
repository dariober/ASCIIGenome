...
public class CommandHelpTest {
  ...
  public void testSplitter() throws InvalidCommandLineException {
    Iterable<String> cmdInputList =
        Splitter.on("&&").trimResults().omitEmptyStrings().split("zi && zo && ylim ");
  ...
  public void canPrintHelpString() throws InvalidCommandLineException {
    ...
    cmd.setAdditionalDescription(
        "Set limits of y axis for all track IDs containing regex."
            + " (This~strecth~of~words~in~brackets~should~stay~together.) New line here -> \n"
            + " ylim 0 na. If regex is omitted all tracks will be captured. Default: 'ylim na na"
            + " .*' Set limits of y axis for all track IDs containing regex. Use na to autoscale to"
            + " min and/or max. E.g. ylim 0 na. If regex is omitted all tracks will be captured."
            + " Default: 'ylim na na .*'");
