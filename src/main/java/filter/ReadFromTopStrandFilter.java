...
/**
 * Extension of samflags to filter reads coming from top strand. Useful for methylation calling.
 * Assign to this filter the next bit integer (4096)
 *
 * @author berald01
 */
  ...
  public boolean filterOut(final SAMRecord record) {
    ...
    boolean isTopStrand =
        ((!record.getReadNegativeStrandFlag() && !record.getReadPairedFlag())
            ...
            (record.getReadNegativeStrandFlag()
                && record.getReadPairedFlag()
                && record.getSecondOfPairFlag())); // -ve 2nd in pair
