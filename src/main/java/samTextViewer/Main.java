...
public class Main {
  ...
  public static void main(String[] args)
    ...
    if (batchFile != null && !batchFile.isEmpty()) {
      ...
      while ((line = br.readLine()) != null) {
        ...
        InteractiveInput itr =
            new InteractiveInput(console, debug, opts.getBoolean("nonInteractive"));
    ...
    while (true) {
      ...
      String memTime = "";
      if (opts.getBoolean("showMemTime")) {
  ...
  /**
   * On shutdown, prepare and write the history file. Not that the existing yaml file is
   * overwritten.
   */
  private static void writeYamlHistory(
      final ASCIIGenomeHistory current,
                ...
                new Runnable() {
                  public void run() {
                    ...
                    try {
                      ...
                      int max_cmds =
                          2000; // Maximum number of commands to write out to asciigenomo_history.
                      while (iter.hasNext()) {
                        if (max_cmds == 0) {
                          break;
                        }
                      ...
                      int max_files =
                          200; // Maximum number of files to write out to asciigenomo_history.
                      lastFiles =
                          lastFiles.subList(
                              Math.max(0, lastFiles.size() - max_files), lastFiles.size());
                      newYamlHistory.setFiles(lastFiles);

                      // Positions
                      List<String> lastPos =
                          gch.prepareHistoryForHistoryFile(newYamlHistory.getFileName(), 100);
                      newYamlHistory.setPositions(lastPos);

                      // Fasta ref
                      if (gch.current().getFastaFile() != null
                          && !gch.current().getFastaFile().trim().isEmpty()) {
                      ...
                      e.printStackTrace();
                      System.err.println(
                          "Unable to write history to " + newYamlHistory.getFileName());
                    }
                  }
                },
                "Shutdown-thread"));
  ...
  private static void messageVersion(final boolean noFormat)
                ...
                new Runnable() {
                  public void run() {
                    try {
                      ...
                      if (cmp == -1) {
                        msg =
                            "NEW: ASCIIGenome version v"
                                + up.get(1)
                                + " is available at "
                                + ArgParse.WEB_ADDRESS
                                + "/releases\n";
                      }
                      if (!noFormat) {
                        msg =
