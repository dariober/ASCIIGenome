...
public class ArgParse {
  ...
  public static String VERSION = "; d097773";
  ...
  public static Namespace argParse(String[] args) {
    ArgumentParser parser =
        ArgumentParsers.newFor(PROG_NAME)
            .build()
            .defaultHelp(true)
    ...
    parser
        ...
        .required(false)
        .help(
            "Bed or gff file of regions to process in batch. Use - to read from stdin.\n"
                + "ASCIIGenome will iterate through the regions in this file");

    parser
        .addArgument("--region", "-r")
        .type(String.class)
        .required(false)
        .help(
    ...
    parser
        ...
        .type(String.class)
        .help(
            "Commands to be executed at the prompt. Either a file with one command per line\n"
                + "a single string of commands, e.g. 'goto chr1 && next && seqRegex ACTG'");

    parser
        .addArgument("--noFormat", "-nf")
        .action(Arguments.storeTrue())
        .help("Do not format output with non ascii chars (colour, bold, etc.)");

    parser
        .addArgument("--nonInteractive", "-ni")
        .action(Arguments.storeTrue())
        .help("Non interactive mode: Exit after having processed cmd line args");

    parser
        .addArgument("--config", "-c")
        .type(String.class)
        .required(false)
        .setDefault("null")
        .help(
            "Source of configuration settings. It can be a local file or a tag matching a\n"
    ...
    parser
        .addArgument("--showMemTime", "-smt")
        .action(Arguments.storeTrue())
        .help(
            "Show memory usage and time spent to process input. Typically used for\n"
                + "debugging only");
    ...
    parser
        ...
        .setDefault(0)
        .help(
            "Set debugging mode. 0: off; 1: print exception stack traces; 2: print stack traces\n"
                + "and exit.");
