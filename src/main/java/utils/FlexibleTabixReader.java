...
public class FlexibleTabixReader implements AutoCloseable {
  ...
  private TIntv getIntv(final String s) {
    ...
    while ((end = s.indexOf(this.columnSeparator, beg)) >= 0 || end == -1) {
      ++col;
      if (col == mSc) {
  ...
  private static final TabixReader.Iterator EOF_ITERATOR =
      new TabixReader.Iterator() {
        @Override
   ...
   */
  public TabixReader.Iterator query(final int tid, final int beg, final int end) {
    TPair64[] off, chunks;
    long min_off;
   ...
   * @return an iterator over the specified interval
   */
  public TabixReader.Iterator query(final String reg) {
    int[] x = parseReg(reg);
   ...
   */
  public TabixReader.Iterator query(final String reg, int start, int end) {
