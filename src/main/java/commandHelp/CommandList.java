...
public class CommandList {
  ...
  private static String reStructuredTextHelp()
    ...
    String intro =
        ".. _command_reference:\n"
            ...
            + "   Do not edit it here. Edit source code then run tests in"
            + " CommandListTest.updateReStructuredFile() to recreate this file.\n\n";
   ...
   */
  public static void updateCommandHelpMdFile(File destFile)
  ...
  public static String briefHelp() throws InvalidCommandLineException, InvalidColourException {
    String help =
        "Available commands with brief description.\n"
            + CMD_HELP
            + "\n"
            + "\n"
            + "      N a v i g a t i o n \n\n";
  ...
  public static final List<CommandHelp> commandHelpList()
      throws InvalidCommandLineException, InvalidColourException {
    ...
    cmd = new CommandHelp();
    cmd.setName("goto");
    cmd.setArgs("chrom[:from[-to]] | chrom [from [to]]");
    cmd.inSection = Section.NAVIGATION;
    cmd.setBriefDescription(
        "Go to region `chrom:from-to` or to `chrom:from` or to the start of `chrom`. ");
    cmd.setAdditionalDescription(
        "The region may be separated by `:` and `-` or by spaces. "
    ...
    cmd.setAdditionalDescription(
        "PERCENT is a number in the range 0-1 mapping to the given percent of the current genomic"
            + " window. Similar to the `:code:INT` command, one number moves the genomic window to"
            + " the position located at PERCENT and two numbers will zoom into the region"
            + " PERCENT-PERCENT.\n"
            + "This command is useful to quickly focus an a feature of interest, such as a ChIP-Seq"
            + " peak or a variant.\n"
            + "\n"
            + "* :code:`c` set the position of *from* at the center of the screen. \n"
            + "\n"
    ...
    cmd.setArgs("INT [k|m]");
    cmd.inSection = Section.NAVIGATION;
    cmd.setPrintName("plus +");
    cmd.setBriefDescription("Move forward by `INT` bases. Suffix K/M recognized. ");
    cmd.setAdditionalDescription(
        "Suffixes k (kilo) and M (mega) are expanded to x1000 and x1,000,000. "
            + "Examples::\n"
            + "\n"
            + "    +2m\n"
            + "    +10k\n"
    ...
    cmd.setArgs("");
    cmd.inSection = Section.NAVIGATION;
    cmd.setPrintName("l - left");
    cmd.setBriefDescription("Go to the Left half of the current window. ");
    cmd.setAdditionalDescription(
        "Alternate the left and right command to quickly focus on a point of interest. ");
    cmdList.add(cmd);
    ...
    cmd = new CommandHelp();
    cmd.setName("next");
    cmd.setArgs("[-back] [-start] [-c] [-zo INT=5] [track]");
    cmd.inSection = Section.NAVIGATION;
    ...
    cmd.setAdditionalDescription(
        "By default `next` centers the window on the next feature and zooms out.\n"
            + "\n"
            + "* :code:`-back` Search backwards. I.e. move to next feature on the left of the"
            + " current position.\n"
            + "\n"
            + "* :code:`-start` Set the window right at the start of the feature, without centering"
            + " and zooming out.\n"
            + "\n"
            + "* :code:`-c` Set the window so that the start of the feature is right in the middle"
            + " of the window. Useful to browse small features such as SNV and indels.\n"
            + "\n"
            + "* :code:`-zo INT` Zoom out INT times after having found the next feature.   Ignored"
            + " if the `-start` flag is set. If <= 0 the window spans exactly the feature"
            + " coordinates.   Default 5.\n"
            + "\n"
            + "* :code:`track` Track to search for next feature. Default to the first annotation"
            + " track found.\n"
            + "\n"
            + "`next` starts searching immediately after the current window and loops thourgh each"
            + " chromosome until a feature is found.");
    cmdList.add(cmd);

    cmd = new CommandHelp();
    cmd.setName("nextChrom");
    cmd.setArgs("[-m] [-M] [regex]");
    cmd.inSection = Section.NAVIGATION;
    ...
    cmd = new CommandHelp();
    cmd.setName("find");
    cmd.setArgs("[-all] [-c] [-F] regex [track]");
    ...
    cmd.setAdditionalDescription(
        "The search for `regex` starts from the *end* of the current window (so the current window"
            ...
            + "\n"
            + "* :code:`-c` Match in CASE SENSITIVE mode. Default is case insensitive (changed in"
            + " v1.12).\n"
            + "\n"
            ...
            + "    find -c 'ACTB gene'~~~~~~-> Find the first match of 'ACTB gene'. Case"
            + " sensitive\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("seqRegex");
    cmd.setArgs("[-iupac] [-c] [regex]");
    cmd.inSection = Section.FIND;
    cmd.setBriefDescription(
        "Find regex in reference sequence and show matches as an additional track. ");
    cmd.setAdditionalDescription(
        "Options:\n"
            + "\n"
            + "* :code:`regex` Regex to search. If missing the seq regex track is removed.\n"
            + "\n"
            + "* :code:`-iupac` Enable the interpretation of the IUPAC ambiguity code. NB: This"
            + " option simply converts IUPAC chracters to the corresponding regex.\n"
            + "\n"
    ...
    cmd.setAdditionalDescription(
        "\n"
            ...
            + "Options:\n"
            + "\n"
            + "* :code:`-p` Pattern to search for (cannot be used with -f). Multiple patterns must"
            + " be separated by comma, without space\n"
            + "\n"
    ...
    cmd.setArgs("[-d] [-n name] [-print] [> file] [chrom:from-to]");
    cmd.inSection = Section.FIND;
    ...
    cmd.setAdditionalDescription(
        "Without arguments, add the current position to the bookmark track. Options:\n"
            + "\n"
            + "* :code:`chrom:from-to` Bookmark this region. If chrom is omitted, use the current"
            + " chromosome.\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("grep");
    cmd.setArgs("[-i = .*] [-e = ''] [-c] [-F] [-v] [track_regex = .*]...");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription(
        "Similar to grep command, filter for features including or excluding patterns.");
    cmd.setAdditionalDescription(
        "Options:\n"
            + "\n"
            ...
            + " v1.12).\n"
            + "\n"
            + "* :code:`-F` Interpret `regex` in `-i` and `-e` as a fixed, literal string instead"
            + " of as a regex.\n"
            + "\n"
            ...
            + "*NOTES*\n"
            + "\n"
            + "* Use *single quotes* to delimit patterns containing spaces e.g. :code:`-i 'ACTB"
            + " gene'`\n"
            + "\n"
            + "Regex `-i` and `-e` are applied to the raw lines as read from source file and it is"
            + " applied only to annotation tracks (GFF, BED, VCF, etc). For example::\n"
            + "\n"
            ...
            + "With no arguments reset to default: :code:`grep -i .* -e ^$ .*` which means show"
            + " everything, hide nothing, apply to all tracks.");
    cmdList.add(cmd);
    ...
    cmd.setAdditionalDescription(
        "awk offers finer control then :code:`grep` to filter records in tabular format.\n"
            ...
            + " expression as delimiter (default delimiter is the TAB character). To access an"
            + " item, i.e. a column, use the syntax :code:`$n` where *n* is the position of the"
            + " item in the list, e.g. :code:`$3` will access the third field (i.e. 3rd column)."
            + " The variable :code:`$0` holds the entire line as single string.\n"
            + "\n"
            + "Awk understands numbers and mathematical operators. With awk you can filter records"
            + " by numeric values in one or more fields since numbers are handled as such. You can"
            + " also perform arithmetic operations and filter on the results.\n"
            + "\n"
            + "*OPTIONS*\n"
            + "\n"
            ...
            + "**ADDITIONAL FEATURES**\n"
            + "\n"
            + "Function :code:`get(...)` can indistinctly be applied to GTF, GFF, SAM records and"
            + " to INFO and FORMAT fields in VCF files. Double quoting around <tag> is optional.\n"
            + "\n"
            + "* :code:`get(tag)` on **GTF**\n"
            + "\n"
            ...
            + "\n"
            + "Return the value of tag attribute. If the attribute contains multiple values return"
            + " the value at index value_idx (1-based). If value_idx is missing (as default),"
            + " return the entire value as it is.\n"
            + "\n"
            + "* :code:`get(tag)` on **SAM**\n"
            + "\n"
            ...
            + "\n"
            + "Return the value of the given **INFO** tag. If the tag contains multiple values,"
            + " optionally return only the value at index *value_index*. If necessary, prepend"
            + " 'INFO/' to tag to disambiguate it from FORMAT tags or if the header does not"
            + " contain this tag. If the tag is of type 'Flag', return 1 if present, 0 otherwise.\n"
            + "\n"
            + "* :code:`get(tag, [sample_idx], [value_idx])` on **VCF**\n"
            + "\n"
            + "Return the value of the **FORMAT** tag for sample index *sample_idx* (default to 1,"
            + " first sample). If the tag contains multiple values, optionally return the value at"
            + " index *value_idx*. If necessary, prepend 'FMT/' to tag to disambiguate it from INFO"
            + " tags or if the header does not contain this tag.  If the tag is of type 'Flag',"
            + " return 1 if present, 0 otherwise.\n"
            + "\n"
            + "* :code:`getAlnEnd()` on **SAM**\n"
            + "\n"
            + "Returns the position of the alignment end. For example, select reads ending after"
            + " position 10000: :code:`getAlnEnd() > 10000`\n"
            + "\n"
            + "* :code:`getAlnLen()` on **SAM**\n"
            + "\n"
            + "Returns the alignment length. For example, select reads with: :code:`getAlnLen() >"
            + " 1000`\n"
            + "\n"
            + "* Column headers\n"
            + "\n"
            + "The following variables are replaced by the appropriate column indexes, so they can"
            + " be used to easily select columns. Make sure the track types are selected to be"
            + " compatible with the headers.\n"
            + "\n"
            + "- bam tracks::\n"
            + "\n"
            ...
            + "*EXAMPLES*\n"
            + "\n"
            + "Note the use of single quotes to wrap the actual script and the use of double quotes"
            + " inside the script.\n"
            + "\n"
            ...
            + "\n"
            + "* Filter for either perfect a match or by matching a regex on 3rd column. Apply to"
            + " all tracks. The second example matches regex on the entire line (similar to grep),"
            ...
            + "\n"
            + "* Filter for features size (assuming bed format) and for values after log10"
            + " transformation. For log10 we need to change base using ln(x)/ln(10)::\n"
            + "\n"
            ...
            + "    awk -off .gtf .gff\n"
            + "\n"
            + "* Return bam records where NM tag (edit distance) is > 0. Double quotes around NM"
            + " are optional::\n"
            + "\n"
            + "    awk 'get(NM) > 0' .bam\n"
            + "\n"
            + "* Filter vcf records by FORMAT tag. Suppose tag AD in the *second* sample is"
            + " :code:`63,7`::\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName(Command.featureDisplayMode.toString());
    cmd.setArgs("[-expanded | -collapsed | -oneline] [-v] [track_regex = .*]...");
    cmd.inSection = Section.DISPLAY;
    ...
    cmd.setAdditionalDescription(
        "\n"
            ...
            + "\n"
            + "* :code:`-oneline/-o` Merge features overlapping on screen coordinates. This option"
            + " makes the track occupy only one line.\n"
            + "\n"
            + "* :code:`-v` Invert selection: apply changes to tracks not selected by list of"
            + " track_regex\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("readsAsPairs");
    cmd.setArgs("[-on | -off] [-v] [track_regex = .*]...");
    ...
    cmd.setAdditionalDescription(
        "If set, properly paired reads in the current window are showed joined up by tildes.\n"
            + "\n"
            + "* :code:`-on|-off` Turn on/off the pairing mode. Or toggle between the two modes if"
            + " none of these flags is set.\n"
            + "\n"
            + "* :code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n"
            + "\n"
            + "* :code:`[track_regex = .*]...` Apply to read tracks captured by these"
            + " regexes.\n");
    cmdList.add(cmd);

    cmd = new CommandHelp();
    cmd.setName("filterVariantReads");
    cmd.setArgs("[-r from/to] [-all] [-v] [track_regex = .*]...");
    cmd.inSection = Section.ALIGNMENTS;
    cmd.setBriefDescription("Filter reads containing a variant in the given interval.\n");
    cmd.setAdditionalDescription(
        ":code:`filterVariantReads` selects for reads where the read sequence mismatches with the"
            + " reference sequence in the given interval on the current chromosome. This command is"
            + " useful to inspect reads supporting a putative alternate allele at a variant site.\n"
            + "\n"
            + "NOTES\n"
            + "\n"
            + "* :code:`filterVariantReads` requires a reference fasta sequence to be set, e.g. via"
            + " the command line option :code:`-fa <ref.fa>` or with command :code:`setGenome`.\n"
            + "\n"
            + "* The CIGAR string determines a mismatch between read and reference. Consequently,"
            + " there may be an inconsistency between variant positions in reads and positions in a"
            + " VCF file if some normalization or indel realignment has been performed by the"
            + " variant caller that generated the VCF. In such cases consider enlarging the target"
            + " interval.\n"
            + "\n"
            + "* The position (POS) of deletions in VCF files refer to the first non-deleted base"
            + " on the reference. Therefore, the interval to :code:`-r` should be POS+1 to filter"
            + " for reads supporting a deletion (but see also the previous point).\n"
            + "\n"
            + "OPTIONS\n"
            + "\n"
            + "* :code:`-r region` Select reads mismatching in this interval. *region* can be given"
            + " as: a single position, a position plus and/or minus an offset, an interval. See"
            + " examples.\n"
            + "\n"
            + "* :code:`-all` Return *all* reads intersecting the :code:`-r` interval, not just the"
            + " variant ones.\n"
            + "\n"
            + "* :code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n"
            ...
            + "    filterVariantReads -r 1000 vars.*vcf <- Apply to tracks captured by"
            + " `vars.*vcf`\n"
    ...
    cmd.setAdditionalDescription(
        "With :code:`gap -on` (default) features which on screen do not have at least one space"
            + " separating them are moved to different lines. In this way it is clear where one"
            + " feature starts and ends. If gap is unset (:code:`gap -off`) features are shown more"
            + " packed.\n"
            + "\n"
            + ":code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n"
            ...
            + "    ||||||||||||\n"
            + "\n"
            + "As elsewhere, this command is applied to all tracks captured by the list of"
            + " regexes.");
    cmdList.add(cmd);

    cmd = new CommandHelp();
    cmd.setName("nameForFeatures");
    cmd.setArgs("[attribute_name = NULL | -na] [-v] [track_regex = .*]...");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription("Select column index or GFF attribute to name features. ");
    cmd.setAdditionalDescription(
        "If the track is of type BED :code:`attribute_name` is expected to be a column index (first"
            + " column has index 1). If track is of type GTF/GFF, :code:`attribute_name` is the key"
            + " from the attributes column (9th column). Use attribute NULL to reset to default"
            + " choice of attribute. To suppress printing of the name use `-na`.Applies to"
            + " annotation tracks captured by the list `track_regex`.\n"
            + "\n"
            + ":code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("trackHeight");
    cmd.setArgs("[-v] INT [track_regex = .*]...");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription(
        "Set track height to INT lines of text for all tracks matching regexes. ");
    cmd.setAdditionalDescription(
        "Setting height to zero hides the track and skips the processing altogether. This is useful"
            + " to speed up the browsing when large bam files are present. Use infoTrack to see"
    ...
    cmd.setAdditionalDescription(
        "Tracks captured by regex(es) are switched BED <-> BEDGRAPH\n"
            + "\n"
            + ":code:`-v` Invert selection: apply changes to tracks not selected by list of"
            + " track_regex\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("ylim");
    cmd.setArgs("[-v] <NUM|min|na> <NUM|min|na> [track_regex = .*]...");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription("Set the y-axis limit for all tracks matched by regexes.");
    cmd.setAdditionalDescription(
        "The first two arguments set the min and max limits. The 3rd argument is a list of regexes"
            + " to capture the tracks to reset. Argument min and max can be:\n"
            + "\n"
            ...
            + " track_regex\n"
            + "\n"
            + "This command applies only to tracks displaying quantitative data on y-axis (e.g."
            + " bigwig, tdf), the other tracks are unaffected. Examples::\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("hideTitle");
    cmd.setArgs("[-on | -off] [-v] [track_regex = .*]...");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription("Set the display of the title line matched by track_regex. ");
    cmd.setAdditionalDescription(
        "Without argument -on or -off toggle between the two modes for all tracks matched by the"
            + " list of regexes.\n"
            + "\n"
            + ":code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n");
    ...
    cmd = new CommandHelp();
    cmd.setName("genotype");
    cmd.setArgs("[-n 10] [-s .*] [-r pattern rplc] [-f expr] [-v] [track_regex = .*]...");
    cmd.inSection = Section.DISPLAY;
    ...
    cmd.setAdditionalDescription(
        "\n"
            ...
            + "\n"
            + ":code:`-r` Edit sample names to replace <pattern> with <replacement>. Names are"
            + " edited only for display. To completely hide names replace with empty string"
            + " :code:`-r .* ''`. To restore original names use a regex matching nothing e.g."
            + " '^$'\n"
            + "\n"
            + ":code:`-f` Filter samples using an expression in Python syntax. See below for"
            + " details.\n"
            + "\n"
            + ":code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n"
            ...
            + "FILTER EXPRESSION\n"
            + "\n"
            + "Samples can be filtered by applying arbitrary expressions to the VCF records. The"
            + " VCF fields of a sample are accessed using the syntax :code:`{TAG}`.\n"
            + "\n"
            + "TAG is one of the fixed fields: CHROM, POS, ID, REF, ALT, QUAL, FILTER, or one of"
            + " the INFO or FORMAT tags. In case of ambiguity, the prefix 'INFO/' or 'FMT/' should"
            + " be used to identify the target tag (e.g. :code:`{FMT/ID}` will access the ID field"
            + " in FORMAT rather than the ID in the header).\n"
            + "\n"
            + "The value(s) in a TAG are converted to the appropriate data type (Integer, String,"
            + " etc). Tags holding more than one value are returned as arrays whose individual"
            + " values should be accessed using the syntax :code:`[index]`. E.g. :code:`{ALT}[0]`"
            + " will access the first alternate allele.\n"
            + "\n"
            + "Note that the ALT and FILTER fields are always arrays, even if only one allele is"
            + " present.\n"
            + "\n"
            + "After substitution of the :code:`{TAG}` placeholders with the actual values, the"
            + " expression string is evaluated as a Python script so any valid Python code is"
            + " allowed including the common operators: :code:`> < == != && ||`.\n"
            + "\n"
            + "Importantly, the result of the expression must be a boolean, i.e. it must evaluate"
            + " to true or false.\n"
            + "\n"
            + "For each sample, the expression is evaluated for each VCF record in the current"
            + " window and if ANY record returns *true*, the sample is filtered-in. To apply the"
            + " filter to specific records either include only those records using e.g. commands"
            + " :code:`grep` or :code:`awk` or make the expression more selective, e.g. by"
            + " including the POS field.\n"
            + "\n"
            + "As elsewhere in ASCIIGenome, if the argument (expression) contains spaces it must be"
            + " enclosed in single quotes and single quotes inside the expression must be escaped."
            + " To remove the expression filter pass a blank string as argument :code:`-f ' '`"
            + " (note the white space between single quotes).\n"
            + "\n"
            + "The following tags can be used to filter on the genotype. When substituted, they"
            + " evaluate to true according to the sample genotype. Testing the :code:`{GT}` tag,"
            + " e.g. :code:`{GT} == \"0/1\"`, achieves a similar result and gives more control but"
            + " using these tags is less error prone:\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("editNames");
    cmd.setArgs("[-t] [-v] <pattern> <replacement> [track_re=.*]...");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription("Edit track names by substituting regex pattern with replacement.");
    cmd.setAdditionalDescription(
        "Pattern and replacement are required arguments, the default regex for track is '.*' (i.e."
            + " all tracks).\n"
            + "\n"
            + "* :code:`-t` (test) flag shows what renaming would be done without actually editing"
            + " the names.\n"
            + "\n"
            + "* :code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n"
            ...
            + "\n"
            + "Use \'\' (empty string in single quotes) to replace pattern with nothing. Examples:"
            + " Given track names 'fk123_hela.bam#1' and 'fk123_hela.bed#2'::\n"
            + "\n"
            ...
            + "    editNames ^ xx_ ~~~~~~~~~~-> xx_fk123_hela.bam#1, xx_fk123_hela.bed#2 (add"
            + " prefix)\n");
    cmdList.add(cmd);

    cmd = new CommandHelp();
    cmd.setName("addHeader");
    cmd.setArgs("[-c] [-a] [-b] [-off] [-v] <header> [track_re=.*]...");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription("Add header to track(s).");
    cmd.setAdditionalDescription(
        "Example use case: You have several tracks sorted in a meanignful way (say WT and CTRL"
            ...
            + " track_regex\n"
            + "\n"
            + "* :code:`<header>` Header text. To change the text format and leave the text as is,"
            + " use :code:`-`. Use :code:`{-}` as placeholder of current header; e.g. add stars"
            + " around existing header: :code:`** {-} **` \n"
            + "\n"
            + "Use :code: `-` for <header> if you want to change the format but leave the text as"
            + " is.\n"
            + "Examples::\n"
            + "\n"
            ...
            + "    addHeader -c cyan -a left - #1 ~~~> Only change colour and alignment in #1 (note"
            + " '-' before #1)\n"
    ...
    cmd = new CommandHelp();
    cmd.setName(Command.print.toString());
    cmd.setArgs(
        "[-n INT] [-full] [-off] [-round INT] [-hl re] [-esf] [-v] [-sys CMD] [track_regex = .*]..."
            + " [>|>> file]");
    cmd.inSection = Section.DISPLAY;
    cmd.setBriefDescription("Print lines for the tracks matched by `track_regex`. ");
    cmd.setAdditionalDescription(
        "Useful to show exactly what features are present in the current window. Features are"
            + " filtered in/out according to the :code:`grep` command. Options:\n"
            + "\n"
            ...
            + "\n"
            + "* :code:`-round INT` Round numbers to this many decimal places. What constitutes a"
            + " number is inferred from context. Default 3, do not round if < 0.\n"
            + "\n"
            + "* :code:`-hl regex` Highlight substrings matching regex. If regex matches a FORMAT"
            + " tag in a VCF record, highlight the tag itself and also the sample values"
            + " corresponding to that tag. Alternatively, regex may be a comma separated list of"
            + " column indexes to highlight. Indexes are recognized by the $ prefix. E.g."
            + " :code:`-hl '$1, $3, $10'` will highlight columns 1, 3, 10.\n"
            + "\n"
            ...
            + " track_regex\n"
            + "\n"
            + "* :code:`-sys` Parse the raw output with the given system command(s). Use"
            + " :code:`-sys null` to turn off the system commands. These commands are executed by"
            + " :code:`bash` so bash is expected to be available on the system. The commands should"
            + " read from stdin and write to stdout, this is usually the case for Unix commands"
            + " like :code:`cut`, :code:`sort`, etc. The command string must be enclosed in single"
            + " quotes, single quotes inside the string can be escaped as \\' (backslash-quote)\n"
            + "\n"
            + "* :code:`>` and :code:`>>` Write output to `file`. `>` overwrites and `>>` appends"
            + " to existing file. The %r variable in the filename is expanded to the current"
            + " genomic coordinates. Writing to file overrides options -n and -off, lines are"
            + " written in full without limit.\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("setGenome");
    cmd.setArgs("fasta|bam|genome");
    cmd.inSection = Section.GENERAL;
    cmd.setBriefDescription("Set genome and reference sequence.");
    cmd.setAdditionalDescription(
        "The genome, i.e. the list of contig names and sizes, "
    ...
    cmd = new CommandHelp();
    cmd.setName("setConfig");
    cmd.setArgs("<file|tag> | <key> <value>");
    cmd.inSection = Section.GENERAL;
    ...
    cmd = new CommandHelp();
    cmd.setName("explainSamFlag");
    cmd.setArgs("INT [INT ...]");
    cmd.inSection = Section.GENERAL;
    cmd.setBriefDescription("Explain the list of bitwise SAM flags. ");
    cmd.setAdditionalDescription(
        "Decode one or more sam flags to human readable form and print them as a table. "
            + "Similar to "
    ...
    cmd.setAdditionalDescription(
        "The argument :code:`arg` takes the following choices:\n"
            + "\n"
            + "* :code:`genome`: Show chromosomes sorted by size\n"
            + "\n"
            + "~~~~* :code:`-n int`: Show up to *int* number of chromosomes or -1 for no limit"
            + " (default 50)\n"
            + "\n"
            ...
            + "\n"
            + "* :code:`pctRuler`: Toggle the display of the column number of the terminal (useful"
            + " for navigation within the current genomic window).\n"
            + "\n"
    ...
    cmd.setAdditionalDescription(
        "Files are listed with their absolute path.\n"
            + "\n"
            + "* :code:`-n INT` Return only the last INT files.\n"
            + "\n"
            + "* :code:`-grep <pattern>` Filter for files (strings) matching pattern. Use single"
            + " quotes to define patterns containing spaces, e.g. :code:`-grep 'goto chr1'`.");
    cmdList.add(cmd);
    ...
    cmd.setAdditionalDescription(
        "*reload* is useful when an input track file is edited by external actions and you want to"
            + " reload it in the current session. This is easier than dropping and re-opening"
            + " tracks with *dropTracks ... && open ...* since track formattings and filters are"
            + " preserved.\n"
            + "\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("dropTracks");
    cmd.setArgs("[-t] [-v] track_regex [track_regex]...");
    cmd.inSection = Section.GENERAL;
    cmd.setBriefDescription("Drop tracks matching any of the listed regexes.");
    cmd.setAdditionalDescription(
        "* :code:`-t` (test) flag only shows which tracks would be removed but do not remove"
            + " them.\n"
            + "\n"
            + "* :code:`-v` Invert selection: apply changes to the tracks not selected by list of"
            + " track_regex\n"
    ...
    cmd = new CommandHelp();
    cmd.setName("orderTracks");
    cmd.setArgs("[track_regex]...");
    cmd.inSection = Section.GENERAL;
    cmd.setBriefDescription("Reorder tracks according to the list of regexes or sort by name.");
    cmd.setAdditionalDescription(
        "Not all the tracks need to be listed, the missing ones follow the listed ones in unchanged"
            + " order. Without arguments sort track by tag name.\n"
            + "For example, given the track list: `[hela.bam#1, hela.bed#2, hek.bam#3,"
            + " hek.bed#4]`::\n"
            + "\n"
            ...
            + "    orderTracks~~~~~~~~~->~name sort~[hela.bam#1, hela.bed#2, hek.bam#3,"
            + " hek.bed#4]\n");
    cmdList.add(cmd);

    cmd = new CommandHelp();
    cmd.setName("posHistory");
    cmd.setArgs("[-n INT=10]");
    cmd.inSection = Section.GENERAL;
    cmd.setBriefDescription("List the visited positions.");
    cmd.setAdditionalDescription(
        "Recorded positions include the current and the previous "
    ...
    cmd = new CommandHelp();
    cmd.setName("history");
    cmd.setArgs("[-n INT] [-grep = .*]");
    cmd.inSection = Section.GENERAL;
    cmd.setBriefDescription("List the executed commands. ");
    cmd.setAdditionalDescription(
        "Commands executed in previous sessions of ASCIIGenome are in \\~/.asciigenome_history\n"
            + "\n"
            + "* :code:`-n INT` Return only the last INT commands.\n"
            + "\n"
            + "* :code:`-grep <pattern>` Filter for commands (strings) matching pattern. Use single"
            + " quotes to define patterns containing spaces, e.g. :code:`-grep 'goto chr1'`");
    cmdList.add(cmd);

    cmd = new CommandHelp();
    cmd.setName(Command.rpm.getCmdDescr());
    cmd.setArgs("[-on | -off] [-v] [track_regex = .*]");
    cmd.inSection = Section.ALIGNMENTS;
    ...
    cmd = new CommandHelp();
    cmd.setName("samtools");
    cmd.setArgs("[-f INT=0] [-F INT=4] [-q INT=0] [-v] [track_re = .*] ...");
    cmd.inSection = Section.ALIGNMENTS;
    cmd.setBriefDescription(
    ...
    cmd.setAdditionalDescription(
        "Useful for stranded RNA-Seq and BS-Seq: bit flag 4096 selects reads mapping to TOP"
            ...
            + "Examples::\n"
            + "\n"
            + "    samtools -q 10~~~~~~~~~~~-> Set mapq for all tracks. -f and -F reset to"
            + " default\n"
    ...
    cmd = new CommandHelp();
    cmd.setName(Command.BSseq.getCmdDescr());
    cmd.setArgs("[-on | -off] [-v] [track_regex = .*]...");
    cmd.inSection = Section.ALIGNMENTS;
    cmd.setBriefDescription("Set bisulfite mode for read tracks matched by regex.");
    cmd.setAdditionalDescription(
        "In bisulfite mode, the characters M and m mark methylated bases (i.e. unconverted C to T)"
            + " and U and u are used for unmethylated bases (i.e. C converted to T). Upper case is"
            + " used for reads on  forward strand, small case for reverse.\n"
            + "\n"
    ...
    // cmd= new CommandHelp();
    // cmd.setName("pileup"); cmd.setArgs("[track_regex = .*]..."); cmd.inSection=
    // Section.ALIGNMENTS;
    ...
    cmd = new CommandHelp();
    cmd.setName("save");
    cmd.setArgs("[>>] [filename = chrom_start_end.txt']");
    cmd.inSection = Section.GENERAL;
    cmd.setBriefDescription("Save screenshot to file as text or pdf format.");
    cmd.setAdditionalDescription(
        "The default file name is generated from the current coordinates and the default format is"
            + " plain text. If the file name has extension '.pdf' then save as pdf. To append to an"
            + " existing file use :code:`>>`. The string :code:`%r` in the file name is replaced"
            + " with the current coordinates. Examples::\n"
            + "\n"
  ...
  protected static List<CommandHelp> getCommandsForSection(Section section)
      throws InvalidCommandLineException, InvalidColourException {
    List<CommandHelp> cmdList = new ArrayList<CommandHelp>();
