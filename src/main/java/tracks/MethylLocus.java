...
class MethylLocus {
  ...
  //	private static final int MIN_DEPTH_ALT= 3; // Min read depth for alternative allele to be taken
  // into account
  //	private static final double MIN_PCT_ALT= 0.01; // Min % of alternative allele to be taken into
  // account
  //	private static final double MIN_PCT_TOT= 0.98; // % (Reference + Alternative) must be above
  // this otherwise set consensus to N.
  ...
  //		this.refBase= Character.toUpperCase((char)faSeqFile.getSubsequenceAt(chrom,
  // locus.getPosition(), locus.getPosition()).getBases()[0]);
  //
  //		makeMethylLocus(locus, refBase);
  //	}

  /**
   * Constructor based on locusInfo and reference sequence as byte[]. Offset is the genomic position
   * of the first base in subRefSeq. I.e. offset=1 if subRefSeq starts from the start of the chrom.
   * offset=n if byte[] is a substring of reference chromosome starting at n.
   */
