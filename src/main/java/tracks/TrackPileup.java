...
public class TrackPileup extends TrackBedgraph {
  ...
  /**
   * Key: Position of the locus. Value: information at this position. NB: This is a hashmap so
   ...
   * the loci map is empty as no information is provided yet. Positions without info (i.e. 0
   * coverage) are not present in the map at all (to save memory and cpu).
   *
   * @throws IOException
   * @throws SQLException
  ...
  private void add(SAMRecord samRecord, int qryFrom, int qryTo, Map<Integer, Locus> accumulator) {
    ...
    int readPos =
        alnBlocks.get(0).getReadStart() - 1; // Leftmost position where the read starts aligning.
      ...
      for (int i = 0; i < block.getLength(); i++) {
        ...
        char base =
            samRecord.getReadBases().length == 0
                ? 'N'
                : (char) samRecord.getReadBases()[readPos - 1];
  ...
  private List<int[]> getRefPositionOfDeletedBlocks(SAMRecord samRecord) {
    ...
    List<int[]> deletedBlocks =
        new ArrayList<int[]>(ndel); // int array contains start and end of deletion
    if (ndel == 0) {
      return deletedBlocks; // No deletetions, nothing to be done.
    }

    // 1S 2M 3D 4M 5D 6M 7S
    // Walk along the cigar string. When an operator consumes read bases, advance the position
    // tracker
  ...
  private char[] getConsensusSequence() throws IOException {
    ...
    for (int pos = this.getGc().getFrom(); pos <= this.getGc().getTo(); pos++) {
      ...
      if (this.loci
          .get(this.getGc().getChrom())
          .containsKey(pos)) { // If containsKey then the position has coverage.
