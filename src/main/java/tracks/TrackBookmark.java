...
public class TrackBookmark extends TrackIntervalFeature {
  ...
  private String positionToGffLine(GenomicCoords gc, String nameForBookmark) {
    ...
    nameForBookmark =
        nameForBookmark.replaceAll(
            "\"", "_"); // This is a hack to prevent troubles getting back name from GFF raw line.

    return gc.getChrom()
        + "\tASCIIGenome\tbookmark\t"
        + gc.getFrom()
        + "\t"
        + gc.getTo()
        + "\t.\t.\t.\tID=\""
  ...
  /**
   * Remove the bookmark matching the exact coordinates of the current position. Bookmarks partially
   * overlapping are not removed.
   *
   ...
   * @throws InvalidGenomicCoordsException
   */
  public void removeBookmark(GenomicCoords bookmarkRegion)
      throws IOException,
          ClassNotFoundException,
          InvalidRecordException,
          SQLException,
