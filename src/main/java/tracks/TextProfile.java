...
class TextProfile {
  ...
  private String strForZeroTop =
      "~"; // Character.toString ((char) 773); // Upperscore, opposite of _
  ...
  public TextProfile(List<Float> yValues, int yMaxLines, Float num1, Float num2) {
    ...
    this.scorePerDot =
        (double) (num2 - num1)
            / ((double) yMaxLines * 2); // * 2 because we use ':' for 2 units in a single line.

    // Shift the yValues to zero by subtracting the ymin or ymax.
    // FIXME: This resetting makes the smallest zero and therefore indistinguishable from "true"
    // zero!
    double offset = 0;
    if (num1 > 0) {
      offset = num1;
    ...
    // Locate zero on y axis. It's silly to generate a sequence just to find the index closest to
    // zero. But anyway...
  ...
  private List<String> prepareYColumn(Double yValue, int yMaxLines, int y0) {
    ...
    Double yPosDotU =
        Math.abs(yValue / this.scorePerDot); // Y positions in dot units, not line units.
  ...
  private void yColumnSanityCheck(List<String> strDepth, int y0, double yValue) {
    ...
    if (yValue
      ...
      for (int i = y0; i < strDepth.size() - 1; i++) {
        ...
        if (i == y0
            && !(x.equals(this.strFor1stNegU)
                || x.equals(this.strForZero))) { // Only allowed char for zero line
          String msg =
              "y0= "
                  + y0
                  + "; yValue= "
                  + yValue
                  + "; Unexpected char in column bar. Got \""
                  + x
                  + "\"; expected \""
                  + this.strFor1stNegU
                  + "\". Bar:\n"
