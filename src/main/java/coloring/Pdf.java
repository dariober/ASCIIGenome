...
public class Pdf {
  ...
  public void convert(File pdfOut, float fontSize, boolean append)
      throws IOException, DocumentException, InvalidColourException {
    ...
    if (append) {
      ...
      Files.move(
          Paths.get(tmpPdf.getAbsolutePath()),
          Paths.get(pdfOut.getAbsolutePath()),
  ...
  private void appendPdf(File in, File dest) throws IOException, DocumentException {
    ...
    Files.copy(
        ...
        Paths.get(template.getAbsolutePath()),
        StandardCopyOption.REPLACE_EXISTING);
  ...
  /**
   * Read ansi formatted file line by lone and convert each line to a iText paragraph.
   *
   * @throws IOException
   * @throws InvalidColourException
   */
  private List<Paragraph> ansiFileToPdfParagraphs(float fontSize)
      throws IOException, InvalidColourException {

    List<Paragraph> paraList = new ArrayList<Paragraph>();

    byte[] encoded =
        this.ansiInput.getBytes(); // Files.readAllBytes(Paths.get(this.ansiInput.toURI()));
    ...
    for (String xv : ansiList) {
      ...
      for (int i = 0; i < xv.length(); i++) {
        ...
        nChars += 1;
        Chunk chunk =
            new Chunk(c, new Font(Font.FontFamily.COURIER, fontSize, Font.NORMAL, fgBaseCol));
  ...
  /**
   * Parse the string x to get the colour for foreground or background. If the input string doesn't
  ...
  /**
   * Return a list the ansi codes found at the beginning of String x. Not the the escape \033 has
   * been already removed from x. extractAnsiCodes("[31;32;33mFOOBAR") -> [31, 32, 33]
   * extractAnsiCodes("FOOBAR") -> [] engineer
   */
